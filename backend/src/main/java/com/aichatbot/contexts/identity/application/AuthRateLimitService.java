...
public class AuthRateLimitService {
    ...
    public RateLimitDecision consumeLoginAttempt(String tenantKey, String ipAddress) {
        ...
        String tenantKeyHash = hashSegment(tenantKey);
        String ipHash = hashSegment(ipAddress);
        String key = appProperties.getAuth().getRateLimitRedisKeyPrefix()
            + tenantKeyHash + ":" + ipHash + ":" + epochMinuteUtc;
        try {
            ...
            log.warn(
                "auth rate-limit redis unavailable, fail-open tenant_hash={} ip_hash={}",
                tenantKeyHash,
                ipHash,
                exception
            );
    ...
    private String hashSegment(String raw) {
        if (raw == null || raw.isBlank()) {
            return "redacted_empty";
        }
        try {
            byte[] digest = MessageDigest.getInstance("SHA-256").digest(raw.trim().getBytes(StandardCharsets.UTF_8));
            // Keep key/log payload short and deterministic while avoiding raw tenant/ip exposure.
