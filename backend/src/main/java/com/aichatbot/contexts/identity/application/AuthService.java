package com.aichatbot.contexts.identity.application;

import com.aichatbot.contexts.identity.domain.AuthSessionOverview;
import com.aichatbot.contexts.identity.domain.AuthSessionRecord;
import com.aichatbot.contexts.identity.domain.AuthUser;
import com.aichatbot.contexts.identity.domain.MfaChallengeRecord;
import com.aichatbot.contexts.identity.domain.UserMfaRecord;
import com.aichatbot.contexts.identity.infrastructure.AuthMfaRepository;
import com.aichatbot.contexts.identity.infrastructure.AuthRepository;
import com.aichatbot.contexts.identity.presentation.dto.AuthTokenResponse;
import com.aichatbot.contexts.identity.presentation.dto.LoginRequest;
...
import com.aichatbot.platform.error.RetryAfterApiException;
import com.aichatbot.platform.observability.TraceGuard;
import com.aichatbot.contexts.identity.security.UserPrincipal;
import com.aichatbot.contexts.operations.application.OpsBlockService;
...
public class AuthService {
    ...
    public AuthIssue login(String tenantKey, LoginRequest request, String clientIp) {
        ...
        UUID tenantId = UUID.fromString(com.aichatbot.platform.tenancy.TenantContext.getTenantId());
        ...
        if (!rateDecision.allowed()) {
            opsEventService.append(
                ...
                1L,
                Map.of(
                    "tenant_key_hash", maskDimensionValue(tenantKey),
                    "ip_hash", maskDimensionValue(normalizeIp(clientIp))
                )
            );
        ...
        if (!isValidPassword(loginId, password)) {
            ...
            opsEventService.append(
                ...
                "auth_login_failed",
                1L,
                Map.of("login_id_hash", maskDimensionValue(loginId))
            );
    ...
    public AuthIssue refresh(String tenantKey, String refreshToken, String clientType, String clientIp, boolean bodyFallbackUsed) {
        ...
        if (authRepository.consumeRefreshSessionByTokenHash(tenantId, refreshHash, now, normalizeIp(clientIp)) != 1) {
    ...
    public void logout(String refreshToken, String reason) {
        ...
        AuthSessionRecord existing = authRepository.findSessionByTokenHash(tenantId, tokenHash).orElse(null);
        authRepository.revokeSessionByTokenHash(tenantId, tokenHash, now, reason == null ? "logout" : reason);
        if (existing != null) {
    ...
    private String maskDimensionValue(String raw) {
    ...
    private AuthIssue issueTokens(AuthUser user, String clientType, String clientIp, String traceId, List<String> recoveryCodes) {
        ...
        opsEventService.append(
            ...
            "auth_login_success",
            1L,
            Map.of("login_id_hash", maskDimensionValue(user.loginId()))
        );
