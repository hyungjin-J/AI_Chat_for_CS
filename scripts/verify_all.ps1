...
Write-Host "[1/12] docker compose up -d"
docker compose -f infra/docker-compose.yml up -d

$artifactDir = "docs/review/mvp_verification_pack/artifacts"
New-Item -ItemType Directory -Force -Path $artifactDir | Out-Null

Write-Host "[0/12] node version policy check"
...
Write-Host "[3/12] frontend build"
...
Write-Host "[5/12] negative tests"
...
Write-Host "[6/12] idempotency redis e2e"
...
Write-Host "[9/10] sse real concurrency limit proof"
...
Write-Host "[10/12] branch protection check (manual/pass)"
...
Write-Host "[11/12] artifact sanitization scan"
...
Write-Host "[12/12] verification pack consistency"
