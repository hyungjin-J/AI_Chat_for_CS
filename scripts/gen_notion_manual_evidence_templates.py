...
def render_patch_template(timestamp_kst: str, targets: list[str]) -> str:
...
def main() -> int:
    ...
    try:
        ...
        print(f"[WARN] {exc}")
        print("No file was overwritten. Resolve manually or rerun with --force.")
        return 1
    ...
    patch_path.write_text(
        render_patch_template(timestamp_kst=timestamp_kst, targets=targets),
        encoding="utf-8",
    )
    ...
    print("")
    print("Next actions:")
    print("1) Apply manual patch to Notion and fill placeholder fields.")
    print("2) Record BLOCKED reason and close evidence in spec_sync_report.md.")
    print("3) Run scripts/check_notion_manual_exception_gate.py before close.")
    return 0
