...
### 6.9 Identifier Type Policy — UUID 정합성 강제 (필수)
> 목적: `uuid = character varying` 류의 타입 충돌 회귀를 구조적으로 차단하고, ID 타입을 레이어별로 일관되게 유지한다.

#### 6.9.1 원칙(Non-negotiables)
- DB의 `uuid` 컬럼은 Java에서 **`java.util.UUID`로 1:1 매핑**한다.
- API/URL(Path/Query)/JSON Body로 들어오는 `*_id`는 문자열일 수 있으나, **Controller(Edge)에서 즉시 UUID로 파싱/검증**하고 아래 레이어에는 **UUID 객체만** 전달한다.
- Service 메서드 시그니처는 `UUID`를 사용한다(내부 로직에서 String id 전달 금지).
- Repository/Mapper 인터페이스는 `UUID`를 사용한다.
- MyBatis는 **UUID TypeHandler(또는 동등한 전역 설정)** 로 바인딩한다.
- SQL에서 `CAST(#{...} AS UUID)` 사용은 기본적으로 **금지**한다.
  - 예외가 필요하면: “왜 필요한지(Why) + 영향 범위 + 대안 검토 + 테스트 증빙”을 포함한 보고서를 남기고 최소화한다(12.3 참조).

#### 6.9.2 실패 처리(필수)
- invalid UUID format(예: `"abc"`):
  - **400** 또는 **422** 중 프로젝트 표준 1개로 **통일**한다(혼용 금지).
- 존재하지 않는 UUID:
  - **404**
- 타 테넌트 리소스 접근(tenant boundary 침범):
  - **403**

#### 6.9.3 회귀 방지 자동 검증(강제 집행 레이어)
가능한 경우 다음을 CI에 포함한다(권장 → 운영상 사실상 필수).
- Mapper 인터페이스에서 `*Id` 파라미터가 `String`으로 선언된 패턴 탐지 시 **CI 실패 처리**
  - 예외는 allowlist로만 허용(예: `loginId` 등 “문자열 컬럼” 근거 포함).
- UUID 관련 쿼리에 `CAST(#{...} AS UUID)` 패턴이 신규 유입되면 **CI 실패 처리**
  - 정말 필요한 예외는 allowlist + 근거 + 최소화.

#### 6.9.4 Definition of Done
- [ ] Controller → Service → Repository → Mapper 전체 경로에서 UUID가 객체로 유지된다.
- [ ] invalid/404/403 시나리오 통합 테스트가 존재하고 PASS한다.
- [ ] 정적 검사(6.9.3)가 CI에서 자동 실행되어 회귀를 차단한다.
...
### 12.3 작업 착수 전 사전 보고서 생성 의무 (Hardening Gate) — 강제 집행 레이어

#### 목적
대규모 리팩토링/구조 변경/타입 변경/보안 관련 수정은 “작업 후 보고”가 아니라  
**작업 전 설계 + 회귀 경로 + 검증 계획(DoD)을 문서로 고정**하여, 실수/누락/회귀를 구조적으로 차단한다.

#### 적용 대상(하나라도 해당되면 필수)
- 타입 체계 변경 (예: `String → UUID`, DTO/Contract 필드 타입 변경)
- DB 구조/쿼리/Mapper/Repository/Service 시그니처 변경
- 보안 관련 수정 (RBAC, Tenant isolation, 인증/인가, PII 차단/마스킹)
- Answer Contract / Fail-Closed / Safe response / SSE 이벤트 계약 변경
- RAG 파이프라인(검색/리랭크/인용/근거 임계치) 변경
- 전역 설정/공통 유틸/빌드/CI 파이프라인 변경
- (스펙 파일 변경은 별도로 2.2/2.2-A 규칙을 우선 적용)

#### 필수 절차(순서 엄수)
1) **코드 수정 전에** 아래 문서를 먼저 생성한다(파일이 없으면 코드 변경 금지).
   - 위치(표준): `docs/review/plans/`
   - 파일명(표준): `YYYYMMDD_<topic>_design_and_hardening_plan.md`
2) 문서에 최소 아래 항목을 먼저 작성한다.
   - 변경 목적(Why) / 배경
   - 영향 범위(파일/레이어/모듈)
   - 회귀 가능 경로(“어디서 다시 터질 수 있는지”)
   - 실패 시 리스크(운영/보안/데이터 정합성)
   - 검증 계획(테스트/회귀/명령어) + Definition of Done(DoD)
   - 예외/트레이드오프(있다면)
3) 위 문서가 작성된 이후에만 구현을 진행한다.

#### 완료 조건(DoD)
- [ ] 사전 보고서(12.3)가 존재한다.
- [ ] 구현/테스트/문서 업데이트가 DoD 기준으로 완료되었다.
- [ ] 변경 후 회귀 방지 장치(정적 검사/테스트/게이트)가 실제로 동작함을 증빙했다.

#### 실패 정의(하나라도 충족하면 “작업 실패”)
- 사전 보고서 없이 고위험 변경을 진행한 경우(또는 코드 수정 후 뒤늦게 형식적으로 작성한 경우)
- 영향 범위/회귀 경로/검증 계획이 누락된 경우
- DoD 없이 PR/커밋을 생성한 경우
- 보고서와 구현이 불일치하는 경우(보고서가 최신이 아님)
...
#### 예외 규정 없음
“이번만 예외”, “단순 리팩토링” 등은 예외 사유가 되지 않는다.  
(단, **순수 문서 오탈자/주석 수정** 등 코드/계약/데이터에 영향이 없는 작업은 적용 대상에서 제외 가능)
...
### 12.4 변경 유형별 최소 체크리스트 (강제)

> 목적: 변경 범위가 커질수록 “뭘 빠뜨렸는지”가 문제다. 변경 유형별 최소 체크를 고정한다.

#### 12.4.1 DB/Mapper/Repository 변경
- [ ] `tenant_key` 필터 누락이 없는지 점검(4.4/6.8.5)
- [ ] `${}` 사용 0건 유지(6.8.1)
- [ ] (UUID 컬럼 포함 시) UUID 정책(6.9) 준수 + CAST 제거
- [ ] 통합 테스트 또는 Testcontainers 기반 테스트로 최소 1개 이상 증빙

#### 12.4.2 API 계약/Controller 변경
- [ ] API 스펙(워크북) 기준과 구현이 일치(6.3)
- [ ] 에러 응답 포맷/상태코드 일관성 유지(5.3, 9.3)
- [ ] trace_id/tenant_key 헤더 전파 누락 없음(4.3, 9.1)

#### 12.4.3 SSE/스트리밍 변경
- [ ] 이벤트 타입 계약 유지(7.2, 9.2)
- [ ] done 이전 종료/재연결/resume 규약 영향 점검(UI-005)
- [ ] fail-closed/safe_response 경로가 자유 텍스트로 우회되지 않음(4.1)

#### 12.4.4 AI/RAG/Answer Contract 변경
- [ ] 인용(citation) 누락률/정책 위반률/Fail-closed 전환률 회귀 점검(8.1~8.2, 10.3)
- [ ] PII 차단이 전 구간 유지(4.2)
- [ ] 대표 시나리오 회귀 테스트(가능하면 자동화)

---

### 12.5 에이전트 프롬프트 헤더 템플릿 (권장)

아래 템플릿을 “모든 Codex/에이전트 작업 프롬프트” 첫 줄에 붙이는 것을 권장한다.
(에이전트가 전역 규칙을 누락하지 않게 하는 가드레일)

```text
[AGENTS.md 준수]
1) AGENTS.md를 먼저 읽고, 이번 작업이 12.3(Hardening Gate) 대상인지 판단한 뒤 근거와 함께 선언하세요.
2) 12.3 대상이면: docs/review/plans/에 사전 보고서(DoD/검증 계획 포함)를 먼저 생성하고, 그 다음에만 코드 수정하세요.
3) 완료 시: backend test / provider regression(있으면) / frontend build 결과와 핵심 변경 파일 목록을 보고서에 남기세요.
4) PII/시크릿은 어떤 형태로도 커밋/로그/예시에 포함하지 마세요.
```

### 12.6 통합 검증(One-Command Check) 스크립트 표준 (권장 → 운영상 사실상 필수)
> 목적: “각자 다른 명령으로 확인”하면 누락/환경차이로 회귀가 발생한다. 검증 경로를 1개로 고정한다.

#### 권장 표준
- Windows: `scripts/check_all.ps1`
- Linux/Mac: `scripts/check_all.sh`

#### 스크립트가 수행해야 하는 최소 작업(권장)
- Backend: `./gradlew test` (또는 레포 표준 명령)
- Provider 회귀(있으면): `scripts/run_provider_regression.ps1` (또는 동등 스크립트)
- Frontend: `npm run build` (또는 레포 표준 명령)

#### 산출물(권장)
- 실행 로그는 `docs/review/**/artifacts/` 하위에 저장하여 재현 가능하게 유지한다.
- 고위험 변경(12.3 대상)일수록 “명령어/로그/결과”를 보고서에 남긴다.

### 12.7 ADR(Architecture Decision Record) 기록 규칙 (권장)
> 목적: 중요한 기술 의사결정을 “구두/기억”에 두면, 시간이 지날수록 맥락이 사라지고 회귀/중복 논쟁이 발생한다.

#### 표준
- 위치(권장): `docs/adr/`
- 파일명(권장): `ADR-YYYYMMDD-<slug>.md`

#### ADR에 포함할 최소 항목(권장)
- Context: 왜 이 결정을 해야 했는가(문제/제약)
- Decision: 무엇을 선택했는가(1문장)
- Alternatives: 대안과 선택하지 않은 이유
- Consequences: 장단점/운영 영향/리스크
- Follow-ups: 후속 작업/검증 항목

#### 연계 규칙(권장)
- 12.3(Hardening Gate) 대상 변경은, 필요 시 관련 ADR 링크를 사전 보고서에 포함한다.
